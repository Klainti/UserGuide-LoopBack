...
class MyTreeStructure {
  ...
  /* Split requested path to get folder until root */
  getFoldersToRoot(path) {
    ...
    while (SplittedPath.length) {
      ...
      FoldersToRoot.push({ path: FolderPath, name: FolderName});
      SplittedPath.pop();
    }
    return (FoldersToRoot);
  }
  ...
  async deleteUp(path) {
    ...
    const FoldersToRoot = this.getFoldersToRoot(path);
    while (i < FoldersToRoot.length){
      if (FoldersToRoot[i].path === '/') {
        descendantsPath = `${FoldersToRoot[i].path}${FoldersToRoot[i].name}`;
      } else {
        descendantsPath = `${FoldersToRoot[i].path}/${FoldersToRoot[i].name}`;
      }
      ...
      if (descendants.length === 0) { // there is no descendants = empty folder -> delete folder!
        await this.FolderM.destroyAll({ path: FoldersToRoot[i].path, name: FoldersToRoot[i].name })
          .catch((error) => {
            return([error, null]);
          });
